package com.yifasilverguard.controller;
...
import com.yifasilverguard.utils.StringUtil;
...
public class LoginController {
    ...
    public Result<?> sendSmsCode(@RequestBody SendSmsCodeDTO sendSmsCodeDTO) {
        if (ObjectUtil.isEmpty(sendSmsCodeDTO.getPhone())
                || !Validator.isMobile(sendSmsCodeDTO.getPhone())) {
            return Result.failure("手机号格式错误");
        }

        loginService.sendSmsCode(sendSmsCodeDTO);
        return Result.success(LoginConstant.SMS_CODE_SENT);
    }
    ...
    public Result<?> bindPhone(@RequestBody BindPhoneDTO bindPhoneDTO) {
        Long userId = BaseContext.getCurrentUserId();
        if (userId == null) {
            return Result.unauthorized();
        }

        if (ObjectUtil.hasEmpty(bindPhoneDTO.getPhone(), bindPhoneDTO.getSmsCode())
                || !Validator.isMobile(bindPhoneDTO.getPhone())) {
            return Result.failure(LoginConstant.BIND_PHONE_ERROR);
        }

        boolean success = loginService.bindPhone(bindPhoneDTO, userId);
        return success ? Result.success(LoginConstant.BIND_PHONE_SUCCESS)
                : Result.failure(LoginConstant.BIND_PHONE_ERROR);
    }
